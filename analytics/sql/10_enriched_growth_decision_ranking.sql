drop view if exists enriched_growth_decision_ranking;

create view enriched_growth_decision_ranking as
...
rank_base as (
    select
        sf.traffic_type,
        coalesce(cle.derived_message_framing, ca.message_framing, sf.first_message_framing, '(unknown)') as derived_message_framing,
        coalesce(cle.derived_audience_segment, ca.audience_segment, '(unknown)') as derived_audience_segment,
        coalesce(cle.derived_cta_style, ca.cta_style, '(unknown)') as derived_cta_style,
    ...
    group by
        sf.traffic_type,
        coalesce(cle.derived_message_framing, ca.message_framing, sf.first_message_framing, '(unknown)'),
        coalesce(cle.derived_audience_segment, ca.audience_segment, '(unknown)'),
        coalesce(cle.derived_cta_style, ca.cta_style, '(unknown)'),
...
select
    traffic_type,
    content_theme,
    derived_message_framing as message_framing,
    derived_audience_segment as audience_segment,
    derived_cta_style as cta_style,
    sessions,
